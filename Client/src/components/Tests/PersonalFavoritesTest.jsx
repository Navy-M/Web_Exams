...
export default function PersonalFavoritesTest({ questions = [], duration = 8, session }) {
  const { user } = useAuth() || {};
  const navigate = useNavigate();
  const { notify } = useNotification();
  const startTimeRef = useRef(new Date(session?.startedAt || Date.now()).getTime());
  const userId = user?.id || user?._id;
  const doneKey = scopedStorageKey(DONE_KEY, userId, "PERSONAL_FAVORITES");
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (res?.user || res?._id || res?.id) {
        notify("آزمون علایق شخصی با موفقیت ثبت شد.", { type: "success" });
